...
exports.listSameBrand = (ProID, count) => {
    var sql = `select * from sanpham where ID != ${ProID} and 
    NhaSanXuat = (select NhaSanXuat from sanpham where ID = ${ProID}) limit ${count}`;
    return db.load(sql);
}

exports.listSameCategory = (ProID, count) => {
    var sql = `select * from sanpham where ID != ${ProID} and 
    Loai = (select Loai from sanpham where ID = ${ProID}) limit ${count}`;
    return db.load(sql);
}